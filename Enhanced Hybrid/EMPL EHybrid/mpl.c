 ...
 */
#if MPL_REACTIVE_FORWARDING //trickle timer pour le domaine (message controle)
...
#define mpl_data_trickle_timer_start_rea(t) { (t)->e = 0; trickle_timer_set(&(t)->tt, data_message_expiration_rea, (t)); }
/**
...
uint8_t j=MPL_BUFFERED_MESSAGE_SET_SIZE - 1,i;
...
//-----------------------Emem------------------------
     //allocating a place in the buffer is like a new msg_used message. (LRU)
	for(i=0; i<MPL_BUFFERED_MESSAGE_SET_SIZE; i++)
	{	
		buffer[i][j]=0;
		buffer[j][i]=1;
	}
...
buffer_reclaim(void)
{
  static uint8_t Zero[MPL_BUFFERED_MESSAGE_SET_SIZE];
  static int max=0, M_ind=0;
  static struct mpl_msg *reclaim;
  uint8_t i,j;

  for(i=0; i<MPL_BUFFERED_MESSAGE_SET_SIZE ;i++) // search for the least used message that is more than zero in the table  
	{
		Zero[i]=0;
		for(j=0; j < MPL_BUFFERED_MESSAGE_SET_SIZE ; j++)
		{
			if(buffer[i][j]==0)
			{
				Zero[i]++;
...
// allocating a place in the buffer is like a new msg_used message. 
	for(i=0; i<MPL_BUFFERED_MESSAGE_SET_SIZE; i++)
	{	
		buffer[i][M_ind]=0;
		buffer[M_ind][i]=1;
   ...
   reclaim = (struct mpl_msg *) &buffered_message_set[M_ind]; // find the message
   
                  //----------------------------
		 if(list_head(reclaim->seed->min_seq) == reclaim)
		    {
				printf("[Buffer]: the lastes message is delated  seq == %u\n", reclaim->seq);
				reclaim->seed->min_seqno.min = list_item_next(reclaim) == NULL ? reclaim->seq : ((struct mpl_msg *)list_item_next(reclaim))->seq;
		    }else{

					printf("[Buffer]: not the lastes message is delated seq == %u && k == %u\n", reclaim->seq,reclaim->seed->min_seqno.k);
					printf("[Buffer]: the lastes is SN = %u\n",((struct mpl_msg *)list_head(reclaim->seed->min_seq))->seq);
					printf("[Buffer]: min_seqence equal to %u\n",reclaim->seed->min_seqno.min);

					reclaim->seed->min_seqno.SN_msg[(int)(reclaim->seed->min_seqno.k)] = reclaim->seq;
					reclaim->seed->min_seqno.k = (reclaim->seed->min_seqno.k + 1);
                                        if(reclaim->seed->min_seqno.k == 50)
					{
  						reclaim->seed->min_seqno.k = 0 ;
					}
                                        printf("the new k is equal to %u\n",reclaim->seed->min_seqno.k); 
    ...
    list_remove(reclaim->seed->min_seq,reclaim);    
...
uint8_t i;
  ...
  for(locssptr = &seed_set[MPL_SEED_SET_SIZE - 1]; locssptr >= seed_set; locssptr--) {
    if(!SEED_SET_IS_USED(locssptr)) {
      locssptr->count = 0;
//-----------------------Emem------------------------
      locssptr->min_seqno.k = 0;
      for(i=0;i < 50;i++)
	{
		locssptr->min_seqno.SN_msg[i] = 0 ; // LRU Deleted messages table
	}
//...................................................
...
//-----------------------Emem------------------------
static uint8_t m_to_ind(struct mpl_msg *msg){ // message to indice dans le tableau

 uint8_t msg_ind = MPL_BUFFERED_MESSAGE_SET_SIZE-1;


  for(locmmptr = &buffered_message_set[MPL_BUFFERED_MESSAGE_SET_SIZE - 1]; locmmptr >= buffered_message_set; locmmptr--) {
    if(MSG_SET_IS_USED(locmmptr)) {
     	if(msg->seq == locmmptr->seq) {
		return msg_ind;
	}
      } 
     msg_ind = msg_ind-1;
...
static void used_msg(struct mpl_msg *msg){ // used event of LRU

printf("[Buffer]: used_msg()\n");

uint8_t num = m_to_ind(msg);
uint8_t i;
...
*/
  ...
  for(locssptr = &seed_set[MPL_SEED_SET_SIZE - 1]; locssptr >= seed_set; locssptr--) {
    if(SEED_SET_IS_USED(locssptr) && locssptr->domain == dom) {
      locsiptr->min_seqno = locssptr->min_seqno.min;
...
**/
      for(locmmptr = list_head(locssptr->min_seq); locmmptr != NULL; locmmptr = list_item_next(locmmptr)) {
        ...
        cur_seq = SEQ_VAL_ADD(locssptr->min_seqno.min, vec_len);
        if(locmmptr->seq == SEQ_VAL_ADD(locssptr->min_seqno.min, vec_len)) {
...
icmp_in(void)
{
  ...
  static uint8_t vector_len; // length
  static uint8_t r_missing; // indicate if this node has at least one new MPL Data Message to offer
  static uint8_t l_missing; // indicate if the source of the received ICMPv6 message has at least one new MPL Data Message to offer
  ...
  /**conslter les seed_id dans la table seed set**/
  for(locssptr = &seed_set[MPL_SEED_SET_SIZE - 1]; locssptr >= seed_set; locssptr--) {
    if(SEED_SET_IS_USED(locssptr) && locssptr->domain == locdsptr) {
      ...
      LOG_INFO_SEED(locssptr->seed_id);
      PRINTF("\n");
      /**consulter les seed_id dans le message icmpv6 one bye one **/
      while(locsiptr <
...
seed_present:
  ...
  while(locsiptr <
    ...
    /**recupérer le victeur de bit du message ICMPv6**/
    switch(SEED_INFO_GET_S(locsiptr)) {
    case 0:
...
/**Le seed existe et il existe quelque messages
  * localiser le bit qui represent le locmmptr (la tete de la liste)
**/
    r = 0;
    if(locmmptr->seq != locsiptr->min_seqno) {
      ...
      /**Pas de chevauchement dans les messages de buffer et ICMPv6**/
      if(r > vector_len || locmmptr == NULL) {
        ...
        if(SEQ_VAL_IS_GT(locmmptr->seq, SEQ_VAL_ADD(locsiptr->min_seqno, r))) {
          ...
          /**il faut reset le temporisateur trickle pour tout les message de ce seed**/
...
//...................................................
          ...
          /**aucun message dans l'ICMPv6 est dans le seed buffer**/
             l_missing = 1;
	...
	**/
     ...
     */
    do { /**Verifier message par message **/
	...
	// printf("[breack]: mpl_trickle_timer_inconsistency seq=%u\n", locmmptr->seq);
//-----------------------Emem------------------------
		used_msg(locmmptr);
//....................................................
        mpl_trickle_timer_inconsistency(locmmptr);
...
/**recuperer le seed id**/
  ...
  printf("Lancer le traitement reactif- le premier data recu\n");
  #if MPL_CONTROL_MESSAGE_TIMER_EXPIRATIONS > 0
  if(!trickle_timer_is_running(&locdsptr->tt)) {
    mpl_control_trickle_timer_start(locdsptr);
  } 
  ...
  if(locssptr) { 
    if(SEQ_VAL_IS_LT(seq_val, locssptr->min_seqno.min)) {// if message seq-number is less than min sequence of this seed
      /* Too old, drop */
      PRINTF("[MPL]:Too old\n");
      UIP_MCAST6_STATS_ADD(mcast_dropped);
      return UIP_MCAST6_DROP;
    }else
	{
		...
		for(i=0;i<50;i++)
		{
			if(seq_val == locssptr->min_seqno.SN_msg[i])
			{
				/* Too old, drop */
    				 printf("[Buffer]: SN > min_SN but is too old\n");
    			 	 UIP_MCAST6_STATS_ADD(mcast_dropped);
     				 return UIP_MCAST6_DROP;
			}
		}
...
/**If PROACTIVE_FORWARDING is TRUE, the MPL Forwarder MUST initialize and start a Trickle timer for the MPL Data Message**/
  ...
  PRINTF("[MPL]:Min Seq Number=%u, %u values\n", locssptr->min_seqno.min, locssptr->count);
...
// printf("[Buffer LRU]: initialition\n");
