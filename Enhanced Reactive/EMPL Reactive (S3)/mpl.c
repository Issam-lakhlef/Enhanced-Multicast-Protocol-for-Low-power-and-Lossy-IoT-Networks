...
icmp_out(struct mpl_domain *dom)
{
  ...
  printf("[MPL]:MPL Control Message Out\n");
...
icmp_in(void)
{
  ...
  static uint8_t r; // To browse the bit vector in ICMPv6
  static uint8_t *vector; // bit vector in ICMPv6
  static uint8_t vector_len; // Length
  static uint8_t r_missing; // Indicates whether this node has at least one new MPL data message to offer
  static uint8_t l_missing; // Indicate whether the source of the received ICMPv6 message has at least one new MPL data message to offer
  ...
  for(locssptr = &seed_set[MPL_SEED_SET_SIZE - 1]; locssptr >= seed_set; locssptr--) {
    if(SEED_SET_IS_USED(locssptr) && locssptr->domain == locdsptr) {
      ...
      LOG_INFO_SEED(locssptr->seed_id);
      PRINTF("\n");
      /**View seed_id in the icmpv6 message one bye one**/
      while(locsiptr <
	  ...
	  printf("mpl_trickle_timer_inconsistency\n");
...
seed_present:
  ...
  while(locsiptr <
    ...
    /**Find the bit vector in the ICMPv6 message **/
    switch(SEED_INFO_GET_S(locsiptr)) {
    case 0:
     ...
     */
/** The seed exists and there are messages
  * Locate the bit that represents locmmptr (the head of the list)
**/
    r = 0;
    if(locmmptr->seq != locsiptr->min_seqno) {
      ...
      /**No overlap in buffer and ICMPv6 messages**/
      if(r > vector_len || locmmptr == NULL) {
        ...
        if(SEQ_VAL_IS_GT(locmmptr->seq, SEQ_VAL_ADD(locsiptr->min_seqno, r))) {
          ...
          /**The trickle timer must be reset for all messages in this seed**/
          PRINTF("[MPL]:Our max sequence number is greater than their max sequence number\n");
          r_missing = 1;
          ...
          /**No message in ICMPv6 is in seed buffer**/
          l_missing = 1;
        }
        goto next;
      }
...
next:
  ...
  /**EMPL S3: l_missing this node has a lack in the buffer**/
  if(l_missing) {
    PRINTF("[MPL]:Inconsistency detected l=%u", l_missing);
...
{
  static seed_id_t seed_id;
  static uint16_t seq_val; // The sequence number of the Data Message
  static uint8_t S; // The seed ID type
...
/**if MPL_DGRAM_OUT: no need for this step since the domain already exists**/
  if(!locdsptr) {
    locdsptr = domain_set_allocate(&UIP_IP_BUF->destipaddr);
    PRINTF("[MPL]:New MPL Domain ");
    PRINT6ADDR(&UIP_IP_BUF->destipaddr);
...
#if MPL_REACTIVE_FORWARDING // After allocation of the domain
    /* Setup new MPL Domain */ // Initialize a trickle timer for control messages
    if(!trickle_timer_config(&locdsptr->tt,
                             MPL_CONTROL_MESSAGE_IMIN,
                             MPL_CONTROL_MESSAGE_IMAX,
                             MPL_CONTROL_MESSAGE_K)) {
  ...
  if(locssptr) { // if the seed exist (oui dans le cas de MPL_DGRAM_OUT)
    if(SEQ_VAL_IS_LT(seq_val, locssptr->min_seqno)) {// if the sequence number of the message is less than min-sequence of that Seed
      /* Too old, drop */
      PRINTF("[MPL]:Too old\n");
      UIP_MCAST6_STATS_ADD(mcast_dropped);
      return UIP_MCAST6_DROP;
...
#if MPL_REACTIVE_FORWARDING
/**   If the MPL Control Message Trickle timer is not running and
...
**/
  ...
  if(!trickle_timer_is_running(&locdsptr->tt)) {
    ...
    /**   If the MPL Control Message Trickle timer is running, the MPL
